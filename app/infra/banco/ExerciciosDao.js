...
ExerciciosDao.prototype.fazerDownload = function (id, callback) {
  this._conexaoDb.query(
    'select exercicios.id_professor, exercicios.id, exercicios_material.file_name from ' +
...
ExerciciosDao.prototype.mostrarListaInfo = function (entrada, callback) {
  this._conexaoDb.query('SELECT * FROM projel.lista where id = ?', entrada, callback);
}; // ok

ExerciciosDao.prototype.abrirRespostaAluno = function (entrada, callback) {
  this._conexaoDb.query(
    'SELECT * FROM resposta WHERE id_exercicios = ? AND id_aluno = ? AND id_sala = ?',
...
ExerciciosDao.prototype.responderExerciciosAluno = function (entrada, callback) {
  this._conexaoDb.query(
    'UPDATE resposta SET resposta = ? WHERE id_aluno = ? AND id_exercicios = ? AND id_sala = ?',
